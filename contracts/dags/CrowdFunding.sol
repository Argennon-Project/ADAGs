...
contract CrowdFunding is ERC20, Administered {
    ...
    uint immutable private redemptionEndTime;
    ...
    function calculateRefund(uint amount) public view returns (uint) {
        ...
        // It's important that we always calcualte the refund amount based on our actual FIAT balance.
    ...
    function buy(uint256 amount) public {
        ...
        if (!canWithdraw) {
            canWithdraw = (config.fiatTokenContract.balanceOf(address(this)) >= config.minFiatForActivation);
        }
    ...
    function withdraw(uint256 amount) public {
        require(canWithdraw, "Withdrawals are not allowed yet.");
